...
export default function ViewProducts() {
  ...
  useEffect(() => {
    axios.post(`${import.meta.env.VITE_API_URL}${import.meta.env.VITE_PRODUCT}/view`, {
      name: filterName,
      parent_category_id: parentCategory,
  ...
  const changeStatus = () => {
    if (checkBoxValues.length > 0) {
      ...
      axios.put(`${import.meta.env.VITE_API_URL}${import.meta.env.VITE_PRODUCT}/change-status`, {
        ids: checkBoxValues,
      })
        .then((result) => {
          if (result.data._status == true) {
  ...
  const deleteRecord = () => {
    ...
    axios.put(`${import.meta.env.VITE_API_URL}${import.meta.env.VITE_PRODUCT}/delete`, {
      ids: checkBoxValues,
    })
      .then((result) => {
        if (result.data._status == true) {
                        ...
                        <div class="flex items-center">
                          <input
                            checked={
                              products.length > 0
                                ?
                                products.length == checkBoxValues.length ? 'checked' : ''
                                :
                                ''
