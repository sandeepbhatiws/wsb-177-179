...
export default function ViewColor() {
    ...
    useEffect(() => {
        axios.post(`${import.meta.env.VITE_API_URL}${import.meta.env.VITE_COLOR}/view`, {
            name: filterName,
            code: filterCode,
            page: currentPage
        })
    ...
    const deleteRecord = () => {
        ...
        axios.put(`${import.meta.env.VITE_API_URL}${import.meta.env.VITE_COLOR}/delete`, {
            ids: checkBoxValues,
        })
        .then((result) => {
            if (result.data._status == true) {
            ...
            <div className="w-full h-[610px]">
                <div className="max-w-[1220px] mx-auto py-2">
                    ...
                    <div className={` rounded-lg border border-gray-300 px-5 py-5 max-w-[1220px] mx-auto mt-10 ${activeFilter ? "hidden" : "block"}`}>
                        <form className="flex" onSubmit={filterRecord} autoComplete='off'>
                                                        ...
                                                        <div class="flex items-center">
                                                            <input
                                                                checked={
                                                                    colors.length > 0
                                                                        ?
                                                                        colors.length == checkBoxValues.length ? 'checked' : ''
                                                                        :
                                                                        ''
