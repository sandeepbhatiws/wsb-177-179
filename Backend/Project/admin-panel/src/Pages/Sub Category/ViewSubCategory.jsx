...
export default function ViewCategory() {
  ...
  useEffect(() => {
    axios.post(`${import.meta.env.VITE_API_URL}${import.meta.env.VITE_SUB_CATEGORY}/view`, {
      name: filterName,
  ...
  }, [filterName, currentPage, parentCategory, apiStatus]);
  ...
  const changeStatus = () => {
    if (checkBoxValues.length > 0) {
      ...
      axios.put(`${import.meta.env.VITE_API_URL}${import.meta.env.VITE_SUB_CATEGORY}/change-status`, {
        ids: checkBoxValues,
      })
        .then((result) => {
          if (result.data._status == true) {
  ...
  const deleteRecord = () => {
    ...
    axios.put(`${import.meta.env.VITE_API_URL}${import.meta.env.VITE_SUB_CATEGORY}/delete`, {
      ids: checkBoxValues,
    })
      .then((result) => {
        if (result.data._status == true) {
  ...
  return (
    <section className="w-full">
      ...
      <div className={`rounded-lg border border-gray-300 px-5 py-5 max-w-[1220px] mx-auto mt-10 ${activeFilter ? "hidden" : "block"}`}>
        ...
        <form onSubmit={filterRecord} autoComplete='off' className="grid grid-cols-[40%_35%_5%] gap-[1%] items-center ">
          <div className="">
                          ...
                          <input
                            checked={
                              subCategories.length > 0
                                ?
                                subCategories.length == checkBoxValues.length ? 'checked' : ''
                                :
                                ''
