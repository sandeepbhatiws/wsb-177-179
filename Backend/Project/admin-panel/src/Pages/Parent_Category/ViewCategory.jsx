...
export default function ViewCategory() {
  ...
  useEffect(() => {
    axios.post(`${import.meta.env.VITE_API_URL}${import.meta.env.VITE_CATEGORY}/view`, {
      name: filterName,
      page: currentPage
    })
      .then((result) => {
  ...
  const changeStatus = () => {
    if (checkBoxValues.length > 0) {
      ...
      axios.put(`${import.meta.env.VITE_API_URL}${import.meta.env.VITE_CATEGORY}/change-status`, {
        ids: checkBoxValues,
      })
        .then((result) => {
          if (result.data._status == true) {
  ...
  const deleteRecord = () => {
    ...
    axios.put(`${import.meta.env.VITE_API_URL}${import.meta.env.VITE_CATEGORY}/delete`, {
      ids: checkBoxValues,
    })
      .then((result) => {
        if (result.data._status == true) {
                        ...
                        <div class="flex items-center">
                          <input
                            checked={
                              categories.length > 0
                                ?
                                categories.length == checkBoxValues.length ? 'checked' : ''
                                :
                                ''
